...
- **Start with the official [Jetpack Compose pathway](https://developer.android.com/courses/jetpack-compose/course)** — Google's free course is the fastest route from zero to the declarative-UI fluency Parts 4–6 assume, and the [Compose docs](https://developer.android.com/develop/ui/compose/documentation) are the reference while you build.
- **Read the platform docs that gate this port:** [Room](https://developer.android.com/training/data-storage/room) (the local library), the [Storage Access Framework](https://developer.android.com/guide/topics/providers/document-provider) (the folder-access model that replaces CB8's direct filesystem reads), [WorkManager](https://developer.android.com/develop/background-work/background-tasks/persistent) (downloads), and the [Play policy center](https://play.google/developer-content-policy/) before you commit to a distribution channel.
- **Read the [CB8 source](https://github.com/s4njee/CB8) with this guide open** — especially `src/shared/` and the server routes; the port is defined by what those files do, not by what this guide says about them.
- **Ship the Phase 1 server client first** — login → library grid → reader → progress sync against a real CB8 server. It exercises every architectural decision (Retrofit models, Coil auth, Readium embedding) with the least code, and Phase 2's local library slots in behind the same interfaces.
- **Sibling guides in this repo:** the [CB8 iOS guide](CB8_IOS_STUDY_GUIDE.md) (the same port, other platform — read both before writing shared-logic code), [Electron](ELECTRON_STUDY_GUIDE.md) (the architecture being ported), and [SQLite](SQLITE_STUDY_GUIDE.md) (what Room wraps).
