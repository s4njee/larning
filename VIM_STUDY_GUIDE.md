...
- **`:find {file}`** opens a file by name searched along your `path` option; with `set path+=**` it fuzzy-finds anywhere under the project (`:find Model<Tab>`).
- **`:grep {pattern}`** searches file *contents* across the project into the **quickfix list**; configure `grepprg` to use `ripgrep` (`set grepprg=rg\ --vimgrep`). Then `:copen` shows results, `<CR>` jumps to one, and `:cnext`/`:cprev` walk them.
- The built-in file explorer is **`:Explore`** (`:Ex`), a navigable directory listing.
- In practice most people add a fuzzy finder — **fzf** or Neovim's **Telescope** (Part 10) — for instant "open any file / search any text" with a few keystrokes.
