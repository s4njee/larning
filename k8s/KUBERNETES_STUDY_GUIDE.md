...
A Kubernetes cluster splits cleanly into two halves whose relationship explains nearly everything about how the system behaves: a **control plane** that holds the desired state and decides what should happen, and a fleet of **worker nodes** that actually run your containers. The crucial thing to understand is that these halves communicate almost entirely through one component, and that the workers *pull* their instructions rather than the control plane *pushing* them — which is what makes the system resilient to nodes coming and going.

At the center of the control plane is the **`kube-apiserver`**, and it is the only component in the entire system that talks to the database. Every other component — the scheduler, the controllers, the kubelet on each node, your `kubectl` — interacts with the cluster by reading from and writing to the API server, never to the database directly. This single-front-door design is what lets the API server be a stateless REST/gRPC service you can scale horizontally and put behind a load balancer: it holds no state itself, so any replica can serve any request. Behind it sits **`etcd`**, the cluster's source of truth — a strongly-consistent key-value store built on the Raft consensus algorithm (the [Distributed Systems guide](../DISTRIBUTED_SYSTEMS_STUDY_GUIDE.md) covers Raft in depth). Every object you create lives in etcd, which means two operational facts follow immediately: lose etcd and you lose the cluster, so you snapshot it on a schedule; and etcd holds your Secrets in cleartext unless you configure encryption at rest, so you do.

The remaining control-plane components are all **controllers** — control loops that watch the API for a kind of object and work to make reality match it. The **`kube-scheduler`** watches for Pods that have been created but not yet assigned to a node, and for each one it runs a two-stage decision: *filtering* (which nodes could even run this Pod, given its resource requests, affinity rules, and taints?) followed by *scoring* (of the survivors, which is best?). The **`kube-controller-manager`** bundles the dozens of built-in controllers — the one that turns a Deployment into ReplicaSets, the one that turns ReplicaSets into Pods, the one that notices a node went unreachable and evicts its Pods, and many more — each an independent reconciliation loop. And the **`cloud-controller-manager`** holds the cloud-provider-specific glue, so that asking for a `LoadBalancer` Service actually provisions a cloud load balancer. (References: [Components](https://kubernetes.io/docs/concepts/overview/components/), [`etcd` overview](https://etcd.io/docs/current/).)
...
Everything in Kubernetes is a resource exposed by the API server, and internalizing that single fact is what makes the system stop feeling magical: there is no special hidden machinery for Deployments versus Services versus your own custom resources — they are all just objects in the same API, manipulated the same way, watched by controllers the same way. Every object identifies its type by a **Group / Version / Kind** tuple — `apps/v1` `Deployment`, `networking.k8s.io/v1` `Ingress` — where the group namespaces related resources and carries a version for evolution, with the empty group (`""`) being the legacy "core" group that holds the oldest kinds like `Pod`, `Service`, and `Node`. You'll see this structure in the `apiVersion` and `kind` fields atop every manifest you write.

The most important structural idea in the whole API is the split between **`spec` and `status`**, because it *is* the reconciliation model made concrete. In `spec` you declare the desired state — "I want three replicas of this image." In `status`, a controller writes the observed state — "two are ready, one is pending." You write `spec`; controllers write `status`; and you never edit `status` directly, because doing so is lying to the controller about reality. **Declarative reconciliation** is then the entire programming model of Kubernetes: you submit desired state, a controller continuously diffs it against actual state and takes action to close the gap, and it never stops — delete a Pod a Deployment owns and the controller notices the gap and recreates one, not as a special "self-healing feature" but as the ordinary consequence of a loop that never assumes its last action stuck. This is why Kubernetes is robust against partial failure in the way the [Distributed Systems guide](../DISTRIBUTED_SYSTEMS_STUDY_GUIDE.md) describes: the system is always re-converging, never assuming a one-shot command succeeded.

Surrounding `spec` and `status` is **`metadata`** — the name, namespace, labels, annotations, owner references, finalizers, and the `resourceVersion` that the API server uses for optimistic concurrency (so two writers can't silently clobber each other). The division of labor within metadata matters: labels are for *selecting* (queryable identity, covered next), while annotations are for arbitrary metadata that tools and humans read but nobody queries on. When you need to discover what your cluster can do, `kubectl api-resources` lists every kind it knows about, and `kubectl explain deploy.spec.template.spec.containers` walks the schema field by field interactively — the two commands that turn "I don't know the field name" from a web search into a one-line answer. (References: [API conventions](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md), [API overview](https://kubernetes.io/docs/concepts/overview/kubernetes-api/).)
...
Most Kubernetes behavior is driven by *relationships between objects*, not by hidden internal state, and the relationships are expressed through a handful of metadata fields you should learn early — because once you can read them, the system's behavior becomes traceable rather than mysterious.

**Labels** are the foundation: they are queryable key-value identity stamped on objects, and they are how the loosely-coupled pieces of Kubernetes find each other. A Service doesn't hold a list of its Pods; it holds a *label selector*, and the set of Pods carrying matching labels *is* its backend — so a Pod joins or leaves a Service simply by gaining or losing a label, with no explicit registration. The same mechanism connects ReplicaSets to the Pods they manage and lets humans filter (`kubectl get pods -l app=web`). Because labels are this load-bearing, a consistent scheme pays off, and the community standard is the `app.kubernetes.io/` set — `name` (the app, e.g. `payments-api`), `component` (the role within it, e.g. `api`, `worker`, `db`), `instance` (the specific install, e.g. `payments-prod`), and `version` (e.g. `1.7.3`) — which makes cross-cutting queries ("everything in the payments-prod release," "every API component cluster-wide") possible.

**Selectors** are the other half of that relationship, and they are best understood as *contracts that are dangerous to change*: a Deployment's selector defines which Pods it considers its own, and a Service's selector defines which Pods receive its traffic. Many selectors are immutable after creation for exactly this reason — quietly changing what a controller owns could orphan running Pods (leaving them unmanaged) or steal Pods from another controller (causing two controllers to fight over them). **Annotations** are the deliberately non-queryable counterpart to labels: free-form metadata that controllers use for options, configuration checksums, and external IDs, and that humans use for descriptions and runbook links — anything you want to attach but never select on.

The last two fields make the object graph navigable. **Owner references** connect parents to children — a Deployment owns its ReplicaSets, which own their Pods — and this tree is what cascading deletion follows: delete the Deployment and garbage collection walks the owner references down to remove the ReplicaSets and Pods (`kubectl tree deploy/foo`, from the `krew` plugin manager, visualizes it). **Finalizers** are the inverse — cleanup hooks that *delay* deletion: when an object sits stuck in `Terminating`, a finalizer is almost always the cause, a controller holding the deletion open until it has cleaned up some external resource (a cloud load balancer, a storage volume) the object represented. Knowing finalizers exist turns "why won't this namespace delete?" from a panic into a diagnosis.
...
A namespace is a scope for resource *names* and a convenient target for attaching RBAC rules and resource quotas — it lets two teams both have a Deployment called `api` without collision, and lets you grant a team rights over everything in their namespace at once. The single most important thing to understand about namespaces, and the most common misconception, is that **a namespace is not a security boundary by itself**: Pods in different namespaces share the same nodes, the same kernel, and the same flat Pod network, so by default a Pod in one namespace can reach a Pod in another unless you add NetworkPolicy, and a container escape crosses namespace lines freely. Namespaces organize and scope; they do not isolate (the [Kubernetes Security guide](KUBERNETES_SECURITY_STUDY_GUIDE.md) covers what real isolation requires).

Not everything is namespaced. Cluster-scoped resources — Nodes, PersistentVolumes, ClusterRoles, and CRDs — exist outside any namespace because they describe the cluster itself rather than a workload within it, while the things you deploy (Pods, Services, ConfigMaps, Deployments) are namespaced. Every cluster ships with a few namespaces by default: `default` (where your objects land if you don't specify one), `kube-system` (the control-plane addons), `kube-public` (world-readable cluster info), and `kube-node-lease` (where nodes write their heartbeats). And namespaces shape DNS: a Service is reachable cluster-wide at `<service>.<namespace>.svc.cluster.local`, but from within the same namespace you can use the short `<service>` name — which is why moving a workload to a new namespace can quietly break a hardcoded short name that now resolves to the wrong place. (References: [Namespaces](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/).)
...
The Pod is Kubernetes' atomic unit of *scheduling* — the smallest thing the scheduler places on a node — but emphatically not the atomic unit of *deployment*, because you almost never create bare Pods in production. A bare Pod is mortal in a way that surprises people: nothing recreates it if its node dies, so production Pods are always created *by* a controller (a Deployment, StatefulSet, or the like) whose whole job is to keep the right number of them alive. Understanding the Pod itself, though, is the foundation everything else builds on.

A Pod is one or more containers that **share a network and IPC namespace**: they see the same `localhost`, the same Pod IP, and the same port space, and they can share volumes — but they do *not* share PID or filesystem namespaces by default, so they remain separate processes with separate filesystems. The shared network namespace is held open by an invisible **`pause` container** that every Pod runs; it does nothing but exist, holding the namespaces so that the real containers can come and go without the Pod losing its identity (the `pause` containers you see in `docker ps`/`crictl ps` on a node are these). Within a Pod, two mechanisms order container startup. **Init containers** run sequentially to completion *before* any app container starts, for setup work like waiting on a dependency or running a database migration — and if any init container fails, the whole Pod restarts, which makes them a clean way to express "don't start the app until this precondition holds." **Sidecar containers** are the modern, stable answer to "a helper that must run *alongside* the app for the app's whole life" (a log shipper, a service-mesh proxy): they're implemented as init containers with `restartPolicy: Always`, default-on since v1.29 and stable since v1.33, and they exist because startup and shutdown *ordering* between an app and its helper matters and the old plain-sidecar approach couldn't guarantee it.
