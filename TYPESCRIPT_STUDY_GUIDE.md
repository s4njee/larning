...
Two developments are reshaping this toolchain picture. First, the checker itself is getting fast: Microsoft is [porting the TypeScript compiler to Go](https://devblogs.microsoft.com/typescript/typescript-native-port/) (the **native port**, `tsgo`, previewable via `@typescript/native-preview`), with roughly 10× faster type-checking; the plan is for the native compiler to ship as **TypeScript 7** while the JavaScript implementation continues in the 6.x line. When it lands fully, the "fast transpiler + slow separate checker" split becomes less painful — but the mental model above (building ≠ type-checking) stays exactly the same. Second, runtimes now execute TypeScript *directly*: [Node.js can run `.ts` files by stripping types](https://nodejs.org/api/typescript.html) (no flag needed since Node 23.6), as Deno and Bun always could — but only for **erasable syntax**, meaning no enums, no `namespace` with runtime code, and no constructor parameter properties, since those compile to real JavaScript rather than deleting cleanly. TypeScript 5.8's `erasableSyntaxOnly` compiler flag enforces exactly that subset, and new projects that enable it keep the option of running their source anywhere without a build step.
