...
Swift uses **Automatic Reference Counting (ARC)** — not garbage collection. The compiler inserts `retain` and `release` calls at compile time; the runtime tracks reference counts and deallocates objects when the count reaches zero. It's deterministic: deallocation happens immediately, with no GC pauses.
