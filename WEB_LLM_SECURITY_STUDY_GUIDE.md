...
The checklist form of this idea is Simon Willison's [**lethal trifecta**](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/): an agent is exploitable when it combines **access to private data**, **exposure to untrusted content**, and **an ability to communicate externally** — because an injection arriving through the untrusted content can direct the model to read the private data and send it out through the external channel. Remove any one leg and that attack collapses: an agent that sees untrusted content but holds no secrets leaks nothing; one that holds secrets but has no exfiltration route (no network tools, and a locked-down rendering surface — Part 11) can't deliver them; one that never ingests untrusted content has no injection vector. Audit every agent you build against the three legs — the trifecta assembles itself quietly as features accrete, and the most dangerous version is an agent that acquired the third leg through an innocuous-looking capability like "browse a URL" or "send a Slack message."
